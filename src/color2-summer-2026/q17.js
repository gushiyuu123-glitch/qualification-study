...
const sourcePrompt = '色みのイメージを前面に打ち出したかったので同系色相でまとめた図1は、［A］配色や［B］配色、さらに［C］配色ということができる。このうちの［B］配色はトーンが同一トーン以外、［C］配色は青紫に近い色相の色よりも、黄に近い色相の色のほうが明るい色でなければならない。この図1をコンプレックス配色にするのであれば、明るいほうの色を下の選択肢の中では［D］の色に変更し、また、スプリットコンプリメンタリーにするのであれば、図1の背景色と図の色の2色はそのままで、文字の色を下の選択肢の中では［E］の色にして3色配色にするとよい。'
...
export const q17 = {
  ...
  answerPage: 18,
  defaultPoints: 3,
  caution: '本試験は記述式。A〜Cは8文字以内のカタカナの配色技法名、D・Eは原本の選択肢番号で解答する。',
  items: [
    {
      part: 'A',
      prompt: sourcePrompt,
