  ...
  if (!colorQualification.categories.some((category) => category.id === CATEGORY_ID)) {
    colorQualification.categories.push({
      ...
      keyPoints: [
        ...
        '誘目性・視認性・明視性・可読性・識別性は、対象を見る段階と目的が異なる。',
        '背景との関係、特に明度差が、見つけやすさや読みやすさへ大きく影響する。',
      ],
      cautions: [
        '誘目性は注意していない対象、視認性は注意して探す対象の発見しやすさ。',
        '明視性は図形、可読性は文字・数字の意味の理解しやすさ。',
        '識別性を高めるための色分けも、色数が多すぎるとかえって区別しにくくなる。',
      ],
    })
  }
}
...
const FOCUS_TERMS_BY_TITLE = {
  色のはたらき: ['機能的な効果と情緒的な効果'],
  誘目性: ['注意を向けていない対象の発見されやすさ'],
  視認性: ['注意を向けて対象を探すときの発見しやすさ'],
  '明視性・可読性': ['図形は明視性、文字・数字は可読性'],
  識別性: ['複数の対象の区別のしやすさ'],
}
  ...
  items: [
    ...
    },
    {
      ...
      intro: [
        ...
        '意味を伝える対象が図形なら明視性、文字や数字なら可読性という。',
      ],
      sections: [
        {
          title: '図形と文字・数字',
...
function applyFocusTerms(shell, title) {
  const focusTerms = [...(FOCUS_TERMS_BY_TITLE[title] ?? [])].sort(
